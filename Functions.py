...
def infected(StudentNumber, Day , DateRange):
    ...
    # Infected people 
        ...
        for SingleInfection in range(round(proportionsSIR[0]*s.StudentObjects[StudentNumber].Social)):
            ...
            StudentNumber = r.randint(0,s.PopulationSize-1)
                             
            if s.StudentObjects[StudentNumber].SIR == 'S':
                s.StudentObjects[StudentNumber].SIR = 'I'
                
                # Adds date of infection as attribute
                s.StudentObjects[StudentNumber].DayInfected.append([DateRange[Day],Day])
            else:
                None
        ...
        if (Day - s.StudentObjects[StudentNumber].DayInfected[-1][-1] ) >= 20:
            s.StudentObjects[StudentNumber].SIR = 'R'
            s.StudentObjects[StudentNumber].DayRecovered.append([DateRange[Day],Day])
        else:
            None
    # Recovery time is 30 days       
    elif s.StudentObjects[StudentNumber].SIR == 'R':
        if (Day - s.StudentObjects[StudentNumber].DayRecovered[-1][-1] ) >= 30:
