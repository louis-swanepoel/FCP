...
InitialInfectionRates = [0.99, 0.01, 0] # where vector is [S,I,R] respectively 

period = [0, 42*24]   # time over which study is done 

infectionRatio = 1    #  percentage of encounters that are infected by a person of status I

Ip = 1/(14*24)      # constant for infection time where infection takes 14 days
Rp = 1/(44*24)      # constant for recovery time where recovery takes a month plus the infected time of 14 days
...
sol = solve_ivp(SIRmodel, period, InitialInfectionRates, t_eval = np.linspace(0, 42*24, num = 1000))
